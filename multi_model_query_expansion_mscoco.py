...
# mscoco
dataset = pd.read_csv('./coco/coco2017/images_captions.csv')
dataset = dataset.sample(frac=0.1, random_state=42) # downsample
data = dataset.to_dict(orient='records')
print(len(data))
query_target_pairs = {item['caption']: item['file_name'] for item in data}
# query_target_pairs

start_index = 0
for idx in tqdm(range(start_index, len(data))):
    record = data[idx]
    multi_modal_query_expansion(record, output_file_path = './mm_rewrite_res/finetuned_blip_result_mscoco.jsonl' )
